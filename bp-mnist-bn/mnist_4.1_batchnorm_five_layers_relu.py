...
def no_batchnorm(Ylogits, is_test, iteration, offset, convolutional=False):
    return Ylogits + offset, tf.no_op()
...
Y1l = tf.matmul(XX, W1)
Y1bn, update_ema1 = batchnorm_layer(Y1l, tst, iter, B1)
Y1 = tf.nn.relu(Y1bn)

Y2l = tf.matmul(Y1, W2)
Y2bn, update_ema2 = batchnorm_layer(Y2l, tst, iter, B2)
Y2 = tf.nn.relu(Y2bn)

Y3l = tf.matmul(Y2, W3)
Y3bn, update_ema3 = batchnorm_layer(Y3l, tst, iter, B3)
Y3 = tf.nn.relu(Y3bn)

Y4l = tf.matmul(Y3, W4)
Y4bn, update_ema4 = batchnorm_layer(Y4l, tst, iter, B4)
Y4 = tf.nn.relu(Y4bn)
...
def training_step(i, update_test_data, update_train_data, max_learning_rate = 0.03, min_learning_rate = 0.0001):
...
for i in range(10000+1):
    a = training_step(i, i % 100 == 0, i % 20 == 0, max_learning_rate=max_lr)
    if not a is None: test_accuracy_list.append(a)
record_file.write("%s Max LR: %f" % (BNorNoBN, max_lr))
for a in test_accuracy_list: record_file.write(",%f" % a)
record_file.write("\n")
...
record_file.close()
    ...
    # early stop
    # if (not a is None):
    #     if (not last_accuracy is None) and (abs(a-last_accuracy) < 0.0000001):
    #         last_accuracy = a
    #         break
    #     last_accuracy = a

# print("max test accuracy: " + str(datavis.get_max_test_accuracy()))
# print("max test accuracy: %f" % float(last_accuracy))
